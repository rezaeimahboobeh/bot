...
define('BOT_TOKEN', '773565947:AAE73ByBtaX8Rwf3NTVZXypOtf8FZGZ7DDk');
...
//  $time = $message['forwarded_'];
  //if (isset($message['text'])) {
    ...
    /*  $sql="SELECT * FROM address WHERE  id=".$text."";
      ...
      $lenght=$row['lenght'];
      $name=$row['name'];*/
      //nearestBranch($latitude,$longitude);
...
define('WEBHOOK_URL', 'https://www.rezaei-mahboobeh.ir/api.php');
...
function nearestBranch($chatid,$latitude,$longitude,$text)
{
  $con = mysqli_connect("localhost","cp35128_bot","botbotbot","cp35128_branch");
  if (mysqli_connect_errno())
    {
...
//    $row = $result->fetch_assoc();
    $latitudeDB=$row['wide'];
    $longitudeDB=$row['lenght'];
    //  $id=$row['id'];
  //  $distance=$latitudeDB+1;
     $distance=pow(($latitude-$latitudeDB),2)+pow(($longitude-$longitudeDB),2);
     $distance=sqrt($distance);
   //$check=$distance;
 if ($distance<=$final) {
     $final=$distance;
     $finallat=$latitudeDB;
     $finallong=$longitudeDB;
   }
    //apiRequestWebhook("sendlocation", array('chat_id' => $chat_id, 'latitude' => $latitudeDB , 'longitude' =>$longitudeDB ));
//$distance=pow(3,2)+pow((4-2),3);
...
function aboutUs($chatid,$text)
{
  /*apiRequestJson("sendMessage", array('chat_id' => $chatid,  'reply_markup' => array(
    'keyboard' => 'about us',
...
function loog($message)
{
  //apiRequest("sendMessage", array('chat_id' => $message['chat']['id'], "text" => $message['message_id'] ));
$id=$message['from']['id'];
...
$text=$message['text'];
  ...
  if (mysqli_connect_errno())
    ...
    $sql="INSERT INTO loog(message_id,first_name, username, tarikh, matn) VALUES ('".$id."','".$first_name."','".$username."','".$date."','".$text."')";
